...
# Statuses where it's too late to change priority
SKIP_STATUSES = {"In Gateway", "Fixed", "Done", "Rejected"}

# Priority ranking: higher number = higher severity
PRIORITY_RANK = {
    "Minor": 1,
    "Major": 2,
    "Critical": 3,
...
def is_priority_change_relevant(current: str, proposed: str, direction: str) -> bool:
    """Check if a proposed change matches the desired direction filter.

    direction: "increase" = only raise severity, "decrease" = only lower, "all" = both
    """
    if direction == "all":
        return True
    current_rank = PRIORITY_RANK.get(current, 0)
    proposed_rank = PRIORITY_RANK.get(proposed, 0)
...
def ticket_should_skip(ticket: dict) -> str | None:
    """Check if a ticket should be skipped.

    Returns a reason string if it should be skipped, or None if it needs analysis.
    """
    if ticket["status"] in SKIP_STATUSES:
        return f"status is '{ticket['status']}' (too late in process)"
...
def run_dry_mode(tickets: list[dict], client: anthropic.Anthropic, severity_ref: str, model: str, config: dict, direction: str = "increase"):
    log_path = config.get("settings", {}).get("log_file", "severity_check_log.json")
    log_data = load_log(log_path)

    log(f"Starting DRY RUN — {len(tickets)} tickets to analyze (direction: {direction}, confidence: high only)")
    print("=" * 100)
    ...
    for i, ticket in enumerate(tickets, 1):
        ...
        skip_reason = ticket_should_skip(ticket)
        if skip_reason:
        ...
        # A change is only raised if confidence is high AND direction matches
        should_raise = (
            analysis["change_needed"]
            and analysis["confidence"] == "high"
            and is_priority_change_relevant(analysis["current_priority"], analysis["proposed_priority"], direction)
        )
        change = "YES" if should_raise else "no"

        if analysis["change_needed"] and not should_raise:
            reason = []
            if analysis["confidence"] != "high":
                reason.append(f"confidence is {analysis['confidence']}")
            if not is_priority_change_relevant(analysis["current_priority"], analysis["proposed_priority"], direction):
                reason.append(f"direction is {direction}")
            log(f"  RESULT: CHANGE DETECTED but filtered out ({', '.join(reason)}) — "
                f"{analysis['current_priority']} -> {analysis['proposed_priority']} [{ticket_elapsed:.1f}s]")
        elif should_raise:
...
def run_actual_mode(
    ...
    log(f"Starting ACTUAL RUN — {total} tickets to analyze (direction: {direction}, confidence: high only)")
    print("=" * 100)
